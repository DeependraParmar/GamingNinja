...
const Videos = () => {
  ...
  return (
    <>
        <Stack direction={['column','row']} p={['0','16']} >
            <VStack w={'full'} alignItems='flex-start' >
                <video controls controlsList='nodownload' src={videoSrc} style={{width: '100%'}}></video>
                <Heading marginY={'4'} >Sample Video</Heading>
                <Text>This is the sample text for the sample demo video 1 which is also called as the description of the video. Lorem ipsum dolor sit amet consectetur adipisicing elit. Impedit ipsam nobis, odit explicabo maiores facere quae blanditiis ad dolorum sint alias fugit, suscipit laudantium dolore exercitationem, fuga et commodi culpa?</Text>
            </VStack>
            <VStack w={['full','xl']} alignItems='stretch' p={'8'} spacing='8' overflowY={'auto'} >
                {
                    videosArr.map((item,itemIndex) => (
                        <Button 
                        variant={'ghost'} 
                        colorScheme='purple' 
                        onClick={()=>setVideoSrc(item)}
                         >Lecture {itemIndex+1}</Button>
                    ))
                }
            </VStack>
        </Stack> 
